...
// renderLogViewerModalView renders the log viewer in a fullscreen modal.
func (m *DashboardModel) renderLogViewerModalView(width, height int) string {
	// Calculate modal dimensions - leave space for borders
	modalWidth := width - 4   // Leave margin for borders
	modalHeight := height - 2 // Leave margin for borders
	...
	if hasActiveFilter {
		if m.filterActive {
			...
			statusParts = append(statusParts, fmt.Sprintf("🔍 Filter: [%s] (%d/%d)",
				m.filterInput.Value(), len(m.logEntries), m.currentTotalLogs()))
		}
	}
