...
// renderSeverityFilterModalView renders the severity filter selection modal.
func (m *DashboardModel) renderSeverityFilterModalView(selected int, width, height int) string {
	// Calculate dimensions - smaller modal
	modalWidth := min(width-16, 50)  // Smaller width for severity list
	modalHeight := min(height-8, 18) // Smaller height
